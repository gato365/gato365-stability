...
mood_dict = {'M-': -1, 'M': 0, 'M+': 1, 'M-e': -1, 'M+e': 1, 'M-l': -1, 'M+l': 1, 'Ml': 0, 'Me': 0}

df['Average_Mood'] = df['Mood'].apply(lambda x: mood_dict[x[0].strip()] if isinstance(x, list) and len(x) > 0 else np.nan)
...
for season in seasons:
    ...
    correlation_goals_weight = season_df[['Average_Goal_30D', 'Weight_30D']].corr().iloc[0, 1]
    correlation_mood_weight = season_df[['Average_Mood_30D', 'Weight_30D']].corr().iloc[0, 1]
    print(f"Correlation between Goals and Weight in {season}: {correlation_goals_weight}")
    print(f"Correlation between Mood and Weight in {season}: {correlation_mood_weight}")

    # Create ratio between mood and goal
    df.loc[df['Season'] == season, 'Mood_Goal_Ratio'] = season_df['Average_Mood_30D'] / season_df['Average_Goal_30D']

colors = ['green', 'red', 'orange', 'blue']
plt.figure(figsize=(10, 5))
for season, color in zip(seasons, colors):
    season_df = df[df['Season'] == season]
    plt.scatter(season_df['Mood_Goal_Ratio'], season_df['Weight_30D'], color=color, label=f'{season} (Mood/Goal)')
plt.xlabel('30-Day Average Goal/Mood')
plt.ylabel('30-Day Average Weight')
plt.title('Seasonal Effects on Correlation between Goals/Mood and Weight')
plt.legend()
plt.show()
